...
#define NULL_VALUE -10000
#define SUCCESS_VALUE 10000
#include <stdio.h>
...
int is_duplicate_ (int idx)
{
    int position;


    position = duplicate items(idx);
    if (position == duplicate items) return 1;
    delete_duplicate_items_at(position) ;
    return SUCCESS_VALUE ;
...
int main(){
    initialize_list();
    is_duplicate(position);
    delete_duplicate_items(position);
    insert_item(10);
    insert_item(20);
    insert_item(30);
    insert_item(40);
